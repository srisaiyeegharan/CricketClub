...
<?php
	...
	else
	{
		...
		if(isset($_POST["coach_fname"])) 
			{
			$coachlname = sanitise($_POST["coach_lname"]);
			$coachfname= sanitise($_POST["coach_fname"]);
			$coachdob= sanitise($_POST["coach_dob"]);
			}
			
			$query = "INSERT INTO $sql_table (CoachFirstName, CoachLastName, CoachDOB) VALUES ('$coachfname', '$coachlname', '$coachdob')";
			$result = mysqli_query($conn, $query);
			if ($result)
			{
				header("location:coach_successful.php");
			}
			else 
			{
