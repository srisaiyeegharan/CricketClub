	...
	else
	{
		...
		if(isset($_POST["player_one_id"])) 
		{
			...
			if (!$result)
				...
				{
					if(isset($_POST["player_2_id"])) 
						{
							$playerr= sanitise($_POST["player_2_id"]);
							$matchh= sanitise($_POST["match_id"]);
							$teamm= sanitise($_POST["team_id"]);
							$positionn= sanitise($_POST["pos2"]);
							$scoree= sanitise($_POST["player_2_score"]);
							$ballss= sanitise($_POST["player_2_balls"]);
							$overss= sanitise($_POST["player_2_overs"]);
							$wicketss= sanitise($_POST["player_2_bowled"]);
					
							
							$query_one = "INSERT INTO $sql_table (MatchId, TeamId, PlayerId, BattingPosition, RunScored, BallsFaced, WicketsTaken, OversBowled) ";
						
							$query_one .=  "VALUES ('$matchh', '$teamm','$playerr', '$positionn', '$scoree', '$ballss', '$wicketss', '$overss')";
